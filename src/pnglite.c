...
int png_open_file_read(png_t *png,
					   const char* filename)
{
...
int png_open_file_write(png_t *png,
						const char* filename)
{
...
static int png_init_deflate(png_t* png,
							unsigned char* data,
							int datalen)
{
    z_stream *stream;
...
static int png_deflate(png_t* png,
					   char* outdata,
					   int outlen, int *outwritten)
{
    int result;
...
static int png_write_idats(png_t* png,
						   unsigned char* data)
{
    unsigned char *chunk;
...
static int png_read_idat(png_t* png, unsigned firstlen)
{
    ...
    do {
		...
		calc_crc =
			crc32(calc_crc, (unsigned char*)"IDAT", 4);
		calc_crc =
			crc32(calc_crc, (unsigned char*)chunk, length);
...
static void png_filter_sub(int stride,
						   unsigned char* in,
						   unsigned char* out,
						   int len)
{
...
static void png_filter_up(int stride,
						  unsigned char* in,
						  unsigned char* out,
...
static void png_filter_average(int stride,
							   unsigned char* in,
							   unsigned char* out,
							   unsigned char* prev_line,
							   int len)
...
static unsigned char png_paeth(unsigned char a,
							   unsigned char b,
							   unsigned char c)
{
    int p = (int)a + b - c;
...
static void png_filter_paeth(int stride,
							 unsigned char* in,
							 unsigned char* out,
							 unsigned char* prev_line,
							 int len)
...
static int png_unfilter(png_t* png, unsigned char* data)
{
    ...
    while(pos < png->png_datalen) {
		...
		switch(filter) {
		case 0: /* none */
			memcpy(data+outpos, filtered+pos,
				   png->width * stride);
			break;
		case 1: /* sub */
			png_filter_sub(stride, filtered+pos,
		...
		case 4: /* paeth */
			if(outpos)
				png_filter_paeth(stride, filtered+pos, data+outpos,
								 data + outpos - (png->width*stride),
								 png->width*stride);
			else
				png_filter_paeth(stride, filtered+pos, data+outpos,
...
int png_set_data(png_t* png,
				 unsigned width, unsigned height,
				 char depth, int color,
				 unsigned char* data)
{
    int i;
    ...
    filtered =
		png_alloc(width * height * png->bpp + height);

    for (i = 0; i < png->height; i++) {
		filtered[i*png->width*png->bpp+i] = 0;
		memcpy(&filtered[i*png->width*png->bpp+i+1],
			   data + i * png->width*png->bpp,
			   png->width*png->bpp);
	}
...
char* png_error_string(int error)
{
	...
	case PNG_WRONG_ARGUMENTS:
		return "Wrong combination of arguments passed to png_open. " \
			"You must use either a read_function or supply a file pointer to use.";
	default:
